	...
	$.videoUI=function(o,p){
		var fn={
				param:{
					...
					selectObjectEvent:false,							//object选中时触发的事件
					objectSelectEvent:"",
											    ...
											    '<div id="ghsp">'+
													'<div id="spnr">'+
														'<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"'+
															...
															'width="1098" height="559">'+
															'<param name="movie" value="Flvplayer.swf" />'+
															'<param name="quality" value="high" />'+
															'<param name="allowFullScreen" value="true" />'+
															'<param name="FlashVars"'+
																'value="vcastr_file=video_2.flv&IsAutoPlay=1" />'+
															'<embed src="Flvplayer.swf"'+
																'allowfullscreen="true"'+
																'flashvars="vcastr_file=video_2.flv"'+
																'quality="high"'+
																'pluginspage="http://www.macromedia.com/go/getflashplayer"'+
																'type="application/x-shockwave-flash" width="1098" height="559"></embed>'+
														'</object>'+
													'</div>'+
												'</div>'+
											'</li>'+
						...
						if(p.selectObjectEvent){
							try{
								if(window.attachEvent){
									...
									/**
									object.get(0).addEventListener(p.selectObjectEvent, function(){li.click();},true);
									**/
									var handler = document.createElement("script");
							  		handler.setAttribute("for", id);
							  		handler.event = p.selectObjectEvent;       
							  		handler.appendChild(document.createTextNode("$('#"+id+"').parent().click();"));
...
//						if(p.videoInit.apply(object.get(0),[object.get(0).dat])){}
				...
				initEvent:function(){
					...
					$this.on("mouseup","div.video-ui-tool div ul li",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
						...
						if(p.videoStopOperate.apply(object.get(0),[object.get(0).dat])){
							
						}
					});
					
					$this.on("mousedown","li.video-ui-tool-zoomIn",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-zoomOut",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-top",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-bottom",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-left",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-right",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-reset",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-start",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-pause",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-zoomOutFocal",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-zoomInFocal",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-openWiper",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-closeWiper",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-zoomOutAperture",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-zoomInAperture",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-printScreen",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
					...
					$this.on("mousedown","li.video-ui-tool-allScreen",function(){
						if(!fn.body){
							alert(p.selectBodyPrompt);
							return false;
						}
