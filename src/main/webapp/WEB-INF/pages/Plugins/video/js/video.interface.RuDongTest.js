window.videoInterface={
		selectObjectEvent:"LButtonDown",
		videoBody:function(){return $("<object classid='CLSID:31AB0C88-6C6E-4924-8E25-C9B3612C3438'></object>");},
		videoInit:function(data){
			//var isInit = this.Init(data.devType);
			var isInit = -1;
			isInit = this.Init(1);			
			if(isInit==0) 
			{
		...
		videoStart:function(data){
			var ret = -1;
			ret = this.PlayByChannel(data.channel);
		...
		videoStopOperate:function(data){console.log("停止操作");var ret = this.PTZStop();return true;},					//停止正在进行的操作
